...
export function MembershipDeck({ cards }: { cards: Offer[] }) {
  ...
  return (
    <div>
          ...
          {cards.map((card, i) => {
            ...
            return (
              <article
                ...
                className={cn(
                  'deck-card noise relative isolate overflow-hidden rounded-[20px] shadow-card inset-shadow-glow',
              ...
              >
                ...
                <div className="flex h-full">
                  <div className="min-w-0 flex-1 p-[28px] lg:w-[calc(544*var(--k))] lg:flex-none lg:p-[calc(66*var(--k))] lg:pl-[calc(76*var(--k))]">
                    <h3 className="font-display text-offer-title leading-[0.96] tracking-[0.1em] text-white uppercase">
                      <Lines text={card.title} />
                    </h3>

                    <p className="mt-[22px] font-sans text-body leading-[1.72] text-white">
                      <strong className="font-semibold">{card.leadIn}</strong>
                      {card.body}
                    </p>

                    <ul className="mt-[30px] space-y-[30px]">
                      {card.bullets.map((b) => (
                        <li key={b} className="flex items-start gap-[23px]">
                          {/* eslint-disable-next-line @next/next/no-img-element -- exported vector */}
                          <img src="/vectors/icon-bullet.svg" alt="" width={22} height={22} className="mt-1 shrink-0" />
                  ...
                  <div className="ml-auto hidden shrink-0 flex-col items-center gap-[34px] p-[calc(24*var(--k))] lg:flex">
      ...
      <div className="mt-[52px] hidden items-center justify-center gap-[30px] lg:flex">
        <button
          type="button"
          onClick={() => step(-1)}
          aria-label="Previous offer"
          className="p-3 transition-opacity hover:opacity-50 motion-safe:active:scale-[0.85]"
        >
          {/* eslint-disable-next-line @next/next/no-img-element -- exported vector */}
          <img src="/vectors/chevron-offers-left.svg" alt="" width={16} height={36} />
        </button>

        <ol className="flex items-center gap-[52px]">
          {cards.map((card, i) => (
            <li key={card.n}>
              <button
                type="button"
                ...
                className={cn(
                  'font-label text-[18px] tracking-[0.1em] transition-colors',
        ...
        >
          {/* eslint-disable-next-line @next/next/no-img-element -- exported vector */}
          <img src="/vectors/chevron-offers-right.svg" alt="" width={16} height={36} />
        </button>
      </div>
    </div>
  );
